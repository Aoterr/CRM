...
var objectId=null;
var field,mlFieldOldValue,mlFieldNewValue,mlFieldOldText,mlFieldNewText,mlFieldNew;
var customer;
var arCityField,arCountyField,arStreetField,arAddrDetailField,arZipCodeField;
var getProvinces,getArea,sourceId,customerTypeCode;
	$(function() {
		...
		var bgData;
		postAjax(false, "${namespacePath}/ModificationDetail/find/all?id=${id}", function(data) {bgData = data}, 'json');
		//变更单 数据表单 接口用法 
		$('#biangeng').datagrid({ 
			fitColumns:true,
			nowrap:false/*, 
		...
		}).datagrid('loadData',bgData).datagrid('acceptChanges');
		...
		$.post('${namespacePath}/modification/find/one', {id :${id}}, function(data) {
			...
			'<security:authorize ifAllGranted="010208">' 
			loadCustomerModificationItemNew();	//加载变更项  高级查询 +关键信息
		    '</security:authorize>';
		}, 'json');
...
// 	}
		...
		if(objectId==null){
			...
			if(field.fdFieldOption == 'text'||field.fdFieldOption == 'date'){
				if(field.fdReserve == 'forever'){
					if($("[name='forever_crExpiryDate']:checked").val()=='0'){mlFieldNewValue = '2099-12-31';mlFieldNewText = '长期';sourceId = sourceIdval();addBianGeng();}else{
			...
			if(field.fdFieldOption == 'select'){mlFieldNewValue = $("#"+mlFieldNew).combobox('getValue');mlFieldNewText = $("#"+mlFieldNew).combobox('getText');sourceId = sourceIdval();addBianGeng();}
			if(field.fdFieldOption == 'checkbox'){mlFieldNewValue = $("input:checkbox[name='"+mlFieldNew+"']:checked").val();mlFieldNewText = $("input:checkbox[name='"+mlFieldNew+"']:checked").attr('disp');sourceId = sourceIdval();addBianGeng();}
			if($("#existFlag").val()==null||$("#existFlag").val()==''){
				if(field.fdFieldOption == 'tel'&&field.fdReserve == '3'){mlFieldNewValue = $("#"+field.fdFieldEn).val();mlFieldNewText = $("#"+field.fdFieldEn).val();sourceId = sourceIdval();addBianGeng();}
				if(field.fdFieldOption == 'tel'&&(field.fdReserve == '1'||field.fdReserve == '2')){
					mlFieldNewValue = telListCreate();
					mlFieldNewText = telsplit(mlFieldNewValue);
				...
				if(field.fdFieldOption == 'addr'){
					if(field.fdReserve=='house'){	
						...
						mlFieldNewValue = addrListCreate();
	...
	function save(state){
		...
		$('#ff').form('submit',{
			url: '${namespacePath}/modification/update?state=' + state,
			success: function(result){
				if (eval('('+result+')').result.success){
					closeCurPageUpdateDatagrid('信息变更','信息变更修改','modification_customer_list');
  ...
  //加载变更项
    ...
    //加载变更项  高级查询 +关键信息
    function loadCustomerModificationItemNew(){
  ...
  //根据类别填充变更项    
   	function loadModificationItem(record) {
   		if(typeof(customerTypeCode)=='undefined' || customerTypeCode=='' ){
		...
		comboboxUtil.setComboboxByUrlWithpanelHeight('biangengxiang', "${ctx}/modification/modification/getBianVal?fdClassEn="+record.fdClassEn+"&customerTypeCode="+customerTypeCode, 'id', 'fdFieldCn', '150', true,function(record){
			...
			if(field.fdFieldOption == 'text'||field.fdFieldOption == 'date'){if(field.fdReserve == 'forever'){
				if($("[name='forever_crExpiryDate']:checked").val()=='0'){mlFieldOldValue = '2099-12-31';mlFieldOldText = '长期';}else{
			...
			if(field.fdFieldOption == 'select'){
				if($("#existFlag").val()==null||$("#existFlag").val()==''){mlFieldOldValue = $("#"+field.fdFieldEn).combobox('getValue');mlFieldOldText = $("#"+field.fdFieldEn).combobox('getText');}
			}
			if(field.fdFieldOption == 'checkbox'){
				if($("#existFlag").val()==null||$("#existFlag").val()==''){mlFieldOldValue = $("input:checkbox[name='"+field.fdFieldEn+"']:checked").val();mlFieldOldText = $("input:checkbox[name='"+field.fdFieldEn+"']:checked").attr('disp');}
			}
			if($("#existFlag").val()==null||$("#existFlag").val()==''){
...
<!-- 				   </tr> -->
   	    ...
   	    <div class="m_btn_box" style="width:820px;">  
            ...
            <a href="#" onclick="closeCurPage('信息变更修改')" class="easyui-linkbutton" >取消</a>   
        </div> 
	</div>
