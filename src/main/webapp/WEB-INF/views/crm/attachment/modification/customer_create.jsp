...
<script type="text/javascript" src="${ctx}/static/swfupload/handlers_create.js"></script>
...
$(function() {
	...
	//填充附件类型
	comboboxUtil.setComboboxByUrlWithpanelHeight('atTypeOne', '${ctx}/crm/param/findByPrTypeAndPrTypename?prType=attachmentType&prTypename='+encodeURI('附件类型'), 'prValue', 'prName', '150', true,function(parent){
		comboboxUtil.setComboboxByUrlWithpanelHeight('atType', "${ctx}/crm/param/findByPrTypeAndPrTypename?prType=attachmentType&prTypename="+parent.id, 'prValue', 'prName', '150', true, function(rec) {
	...
	<div data-options="region:'center'" >
	    <form id="ff" method="post" action="${namespacePath}/save">
		    ...
		    <table  class="m_table" style="width:888px;">
		    	<tr>
					<th class="title" colspan="8">新增附件</th>
				</tr>
			   <tr>
					<td><label for="atType">附件类型</label></td>
					<td><input id="atTypeOne" class="easyui-combobox" name="atTypeOne"/>
						<input id="atType" class="easyui-combobox" name="atType"/>
					</td>
			   </tr>
			   <tr>
			   ...
			   <tr>
					...
					<td colspan="3"><textarea name="atMemo" id="atMemo" style="height:80px;width: 600px;"></textarea></td>
			   </tr> 
			</table>
...
<!-- 		<button>取消所有</button> -->
		<a href="${namespacePath}/show/customer/attachment/modification?customerid=${customerid}&id=${id}&type=6" class="easyui-linkbutton">返回上一页</a>
	</div>
</body>
